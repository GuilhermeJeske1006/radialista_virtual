...
export default function VoiceSelect({ value, onChange }: Props) {
  ...
  return (
    <div>
            ...
            {vozesClonadas.map((v) =>
              editandoId === v.id ? (
                <div key={v.voz_id} className="flex items-center gap-2 px-3 py-2 text-sm">
                  <input
                    autoFocus
                    className="min-w-0 flex-1 rounded-lg border border-border-strong bg-bg px-2 py-1 text-sm text-fg focus:outline-none focus:border-roxo-claro/50 focus:ring-2 focus:ring-roxo-claro/20"
                    value={nomeEdicao}
                ...
                <div key={v.voz_id} className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-fg/5">
                  ...
                  {v.requer_verificacao && <button type="button" className="text-xs text-roxo-claro" onClick={() => setConfigVoz(v.voz_id)}>Verificar status</button>}
                  {v.preview_url && (
      ...
      <button type="button" className="mt-2 text-xs text-roxo-claro" onClick={() => setConfigVoz(value)}>Ajustar voz selecionada</button>
        ...
        {permiteClonagemVoz(plano) ? (
          ...
          plano && (
            <p className="text-xs text-fg/65">
              ...
              <Link href="/billing" className="text-roxo-claro hover:underline">
                Fazer upgrade
              </Link>
            </p>
          )
