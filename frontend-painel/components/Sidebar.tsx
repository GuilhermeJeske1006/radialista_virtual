...
import { OndaLogo, OndaMark } from "./OndaLogo";
    ...
    >
      <div className="flex flex-col flex-1 min-h-0">
        <div className={`py-8 shrink-0 ${colapsada ? "px-0 flex justify-center" : "px-6"}`}>
          {colapsada ? <OndaMark size={28} /> : <OndaLogo />}
        </div>
        {!colapsada && (
        ...
        <nav className={`flex-1 overflow-y-auto space-y-5 ${colapsada ? "px-3 pt-2" : "px-4"}`}>
          {groups.map((group) => (
            <div key={group.label}>
              {!colapsada && (
                <div className="text-xs font-medium uppercase tracking-wide text-fg/40 px-3 mb-2 font-mono">
                  {group.label}
                </div>
              )}
