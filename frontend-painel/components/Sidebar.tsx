...
const DIAL_SCALE = ["88", "90", "92", "94", "96", "ONDA", "100", "102", "104", "106", "108"];

export default function Sidebar() {
  const pathname = usePathname();
  const router = useRouter();
  const conta = useConta();
  const links = LINKS.filter((link) => !link.adminOnly || conta?.role === "admin");
  ...
  return (
    <aside className="hidden md:flex md:w-72.5 md:flex-col md:fixed md:inset-y-0 bg-surface border-r border-border">
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="flex justify-between font-mono text-[10px] tracking-wide text-fg/40 border-y border-border px-6 py-2 mb-4">
          {DIAL_SCALE.map((tick) => (
            <span key={tick} className={tick === "ONDA" ? "text-amber" : ""}>
              {tick}
            </span>
          ))}
        </div>
        <nav className="flex-1 px-4 overflow-y-auto">
          <div className="text-xs font-medium uppercase tracking-wide text-fg/40 px-3 mb-3 font-mono">Menu</div>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                    active ? "bg-amber/10 text-amber" : "text-fg/65 hover:bg-paper/5 hover:text-fg"
                  }`}
                >
                  <svg
                    className={`h-5 w-5 shrink-0 ${active ? "text-amber" : "text-fg/35"}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="currentColor"
        ...
        <div className="px-4 pb-6 space-y-1">
          <Link
            href="/perfil"
            className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
              pathname === "/perfil" ? "bg-amber/10 text-amber" : "text-fg/65 hover:bg-paper/5 hover:text-fg"
            }`}
          >
            <svg
              className={`h-5 w-5 shrink-0 ${pathname === "/perfil" ? "text-amber" : "text-fg/35"}`}
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
          ...
          >
            <svg className="h-5 w-5 shrink-0 text-fg/35" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
